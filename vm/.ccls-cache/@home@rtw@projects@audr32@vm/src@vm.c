...
// #define CLOCKSPEED 20000000 // 20MHz
#define FPS 60
#define TPF 1
...
void dofadd(opcodepre_t);
void dosub(opcodepre_t);
void dofsub(opcodepre_t);
void dodiv(opcodepre_t);
void dofdiv(opcodepre_t);
void doidiv(opcodepre_t);
void domul(opcodepre_t);
void dofmul(opcodepre_t);
    ...
    switch(opcodeprefix.instruction) {
        ...
        case OP_ADD:
            if(opcodeprefix.mode >= 0x00 && opcodeprefix.mode < 0x06) {
                opcodeprefix.mode -= 0x00;
                doadd(opcodeprefix);
            } else if(opcodeprefix.mode >= 0x06 && opcodeprefix.mode < 0x0C) { // floating
                opcodeprefix.mode -= 0x06;
                dofadd(opcodeprefix);
            }
            return NORMOP; 
        ...
        case OP_DIV:
            if(opcodeprefix.mode >= 0x00 && opcodeprefix.mode < 0x06) {
                ...
                opcodeprefix.mode -= 0x06;
                doidiv(opcodeprefix);
            } else if(opcodeprefix.mode >= 0x0C && opcodeprefix.mode < 0x12) {
                opcodeprefix.mode -= 0x0C;
                dofdiv(opcodeprefix);
            }
            return NORMOP;
        case OP_MUL:
    ...
    while(vm.running) {
        ...
        for(size_t i = 0; i < dueticks; i++) {
            ...
            while(cyclesleft > 0) {
                ...
                // printf("opcode: (ip: 0x%08x) 0x%02x|0x%02x (%s) (AX: 0x%08x/%d, BX: 0x%08x/%d, CX: 0x%08x/%d, DX: 0x%08x/%d)\n", vm.regs[REG_IP], opcodeprefix.instruction, opcodeprefix.mode, resolveinstruction(opcodeprefix.instruction), vm.regs[REG_AX], vm.regs[REG_AX], vm.regs[REG_BX], vm.regs[REG_BX], vm.regs[REG_CX], vm.regs[REG_CX], vm.regs[REG_DX], vm.regs[REG_DX]);
